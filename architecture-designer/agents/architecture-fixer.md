...
another. Note the outcome (**USED** with a quoted excerpt, **NOT APPLICABLE**, or **UNAVAILABLE**) in the fix log's
matching "Applied fixes" line. Omit this entirely for a fix where no
`agentTools` entry's domain is relevant, or when `agentTools` was empty or absent.
