...
- **Stage 5 — Technology selection** — opens by selecting 3–6 ranked Architectural Drivers from the Quality Attribute
  ...
  driver ID (s) it satisfies; closes with an ATAM-style Trade-off and Risk Analysis pass recording where drivers
  conflict and what risks the decisions carry; optionally records which MCP servers/Skills available in the environment
  match the chosen stack, for use by downstream design-review and implementation sub-agents (see `agent-tools.md` below)
...
- **Step 10 — Low-Level Design** — API contracts (per sequence diagram endpoint), business rules (pseudocode for
  ...
  in between run architecture review, browser preview, and user confirmation)
...
| `architecture-designer:document-reviewer`        | Audits saved documents for format compliance (F1–F7) and content completeness (C1–C16, including core features, IaC, CI/CD, decentralized-architecture, offline-first, domain-model, and trade-off/risk-analysis sections); returns DOCUMENT REVIEW PASSED / FAILED                                                                                                                                                                    |
...
| `document-template.md`           | The 11-section architecture document body template (Step 11) — including a Core Features section linking each core feature to its dedicated sequence diagram — plus an optional 12th decentralized-architecture section, an optional 13th offline-first-considerations section, and required 14th (Domain Model) and 15th (Trade-off and Risk Analysis) sections                        |
| `document-review-checklist.md`   | The F1–F7 / C1–C16 document review item catalog and literal formats shared by `document-reviewer` and `document-fixer`                                                                                                                                                                                                                                                                |
