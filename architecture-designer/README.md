...
- **Stage 5 — Technology selection** — opens by selecting 3–6 ranked Architectural Drivers from the Quality Attribute
  ...
  and what risks the decisions carry, an Alternatives Considered table (chosen option, rejected option (s), and why each
  lost) for every structural or driver-cited decision — see `critical-thinking-guide.md` — and a Cost Estimation pass
  producing a per-component monthly/annual infrastructure cost breakdown — always WebSearch-attempted first against
  current cloud pricing, falling back to "estimate — verify at implementation time" only when the search genuinely
  returns nothing usable, reconciled against Stage 3's budget (see `cost-estimation-guide.md`); optionally records which
  MCP servers/Skills available in the environment match the chosen stack, for use by downstream design-review and
  implementation sub-agents (see `agent-tools.md` below)
...
| `architecture-designer:implementation-planner`   | Resolves implementation ambiguities, checks for a matching official project generator, proposes a folder structure — deriving one checklist item per ERD entity, per API route group, per IaC module, per CI/CD pipeline config file, and per bounded context's module/service directory name — waits for confirmation, and saves the implementation plan; does not write application code or run the generator itself                                                                                                  |
...
| `adr-guide.md`                   | Architecture Decision Record generation (Step 11) — which Stage 5 decisions and trade-offs qualify, the Nygard-style ADR template with driver-ID traceability and an Alternatives Considered section copied verbatim from `stage5.alternativesConsidered`, file naming/numbering, and the supersede-don't-edit revision procedure                                                                                                                                                                                                                                                    |
