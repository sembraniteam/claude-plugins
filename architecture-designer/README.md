...
- **Stage 2 — Requirements analysis** — functional vs non-functional requirements (performance, security, scalability,
  ...
  response, response measure) per `quality-driven-design-guide.md`
...
- **Stage 5 — Technology selection** — opens by selecting 3–6 ranked Architectural Drivers from the Quality Attribute
  Scenarios and stages 1–4 (per `quality-driven-design-guide.md`), then proposes architecture pattern, backend,
  frontend, database, infrastructure, supporting services, authentication approach, observability strategy, DR approach,
  error handling/resilience strategy (retry, circuit breaker, timeouts, graceful degradation), and rate-limiting
  strategy (algorithm, middleware library, enforcement layer, per-tier limits, distributed store if horizontally
  scaled); every choice justified against stages 1–4 and the driver ID (s) it satisfies; closes with an ATAM-style
  Trade-off and Risk Analysis pass recording where drivers conflict and what risks the decisions carry; optionally
...
- **Stage 6 — Architecture and infrastructure design** — Domain-Driven Design bounded-context/aggregate modeling (see
  `ddd-guide.md`), database schema (ERD, index plan, engine selection, migration strategy), IaC tool selection and
  module structure, CI/CD pipeline design (platform, stages, branching strategy, environment promotion — both skipped
  for a project with no deployment target at all, e.g. a library or CLI tool), and Mermaid diagrams rendered in the
  browser with zoom/pan/download
- **Step 10 — Low-Level Design** — API contracts (per sequence diagram endpoint), business rules (pseudocode for
  non-trivial logic), DTOs, inter-service contracts (microservices/event-driven only), and error catalog (Steps 7–9 in
  between run architecture review, browser preview, and user confirmation)
...
- `docs/architecture-designer/architecture/{yyyymmdd}-{topic}.md` — complete, reviewed, and approved architecture
  document including IaC plan, CI/CD pipeline design, and LLD section
...
then saves a new versioned document through the same document-reviewer pass.
...
corrections, then re-runs the reviewer. This loop runs until the reviewer passes — no manual editing required.
Implementation follows a similar two-step split: `implementation-planner` produces and confirms the plan, then
`architecture-implementer` executes it — the implementer never runs without a plan the planner has already saved.
...
| `architecture-designer:document-reviewer`        | Audits saved documents for format compliance (F1–F7) and content completeness (C1–C15, including IaC, CI/CD, decentralized-architecture, offline-first, domain-model, and trade-off/risk-analysis sections); returns DOCUMENT REVIEW PASSED / FAILED                                                                                                                                                                                   |
...
`@mermaid-js/parser` for new types. Diagrams validated only by heuristics (when parsers are unavailable) are marked
...
targeting itself: only a plan file whose on-disk `Status` currently reads `Complete` is re-verified, and only
architecture-implementer's own verification pass ever writes that value (`implementation-planner` always saves
`Status: In progress`; every other agent never touches a plan file at all). This makes the script self-limiting to the
one completion claim worth re-checking, without depending on a platform capability that isn't confirmed to exist for
this event.
...
common chain toolchains, not an exhaustive list of every deploy mechanism across every ecosystem.
...
`checkpoint-reminder.sh`'s case, to also build the JSON `additionalContext` output) in addition to the Node.js/Python 3
runtimes `validate-diagrams.mjs`/`validate-session.py` already need — see "Scripts" above. Every command hook degrades
to a silent no-op (`exit 0`) rather than failing when a required runtime or `session.json` is entirely missing, so a
project not using this plugin's workflow — or missing `jq`/`node`/`python3` — never sees spurious output or a blocked
tool call from it for that reason. This does not extend to a *referenced* plan file being missing or stale while
...
throughout a project's lifetime. It holds the confirmed answers from Stages 1–6c (`stage1`–`stage6c`), an optional
`agentTools` list, an optional `architecturalDrivers` list and `riskRegister` list (the Stage 5 ADD/ATAM Architectural
Driver selection and Trade-off/Risk Analysis pass — see `skills/design/references/quality-driven-design-guide.md`), an
optional `domainModel` object (bounded contexts and aggregates from Stage 6a's Domain-Driven Design pass — see
...
`pending`, `progress`, and `lld` checkpoint everything between Stage 1 and Step 13 so a session that dies or gets
compacted mid-workflow doesn't lose the expensive parts — see "Mid-workflow persistence" below.

Full schema, the single-writer-per-key rule (each key has exactly one skill/agent that may mutate it, with thirteen
exceptions: `documents` is append-only, legitimately appended to by both `design` and `review` since neither ever
touches the other's entries; `web3`, `offlineFirst`, `stage6b`, `stage6c`, `progress`, `pending`,
`architecturalDrivers`, `riskRegister`, `domainModel`, `stage2.qualityAttributeScenarios`, `stage5.tradeoffAnalysis`,
and
`lld` each have one authorized second writer — `review` overwrites them (in full, or field-by-field/group-by-group for
`progress`/`lld`) on a revision that changes decentralization status, offline-first status, infrastructure provider, IaC
tool, CI/CD platform, an NFR, a technology decision/requirement tension, a bounded-context/aggregate-boundary change, an
API contract/business rule/DTO/inter-service contract/error condition, or that resumes/gathers a mid-pipeline step —
safe because `design` and `review` never run concurrently within one conversation, not because they're append-only), and
the no-CAS read-fresh-modify-write-whole discipline (a usage invariant assuming one session at a time against a given
project directory, not something the plugin mechanically enforces) are documented in
...
- **`session.json`'s `progress` key** tracks an `owner` (`design` or `review` — whichever skill's pipeline pass this
  ...
  exists yet when Stage 6a runs, its entry holds the actual approved schema/ERD/index plan/connection-config text
  directly (`approvedOutput`) instead of a hash of an external file — durable and unambiguous rather than resting on a
  moving target.
...
| `document-template.md`           | The 10-section architecture document body template (Step 11), plus an optional 11th decentralized-architecture section, an optional 12th offline-first-considerations section, and required 13th (Domain Model) and 14th (Trade-off and Risk Analysis) sections                                                           |
| `document-review-checklist.md`   | The F1–F7 / C1–C15 document review item catalog and literal formats shared by `document-reviewer` and `document-fixer`                                                                                                                                                                                                    |
