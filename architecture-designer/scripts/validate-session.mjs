 ...
 * Verifies that stages 1–5 have been confirmed and written before Stage 6 begins.
 * Usage: node scripts/validate-session.mjs
 *
 * Exit 0 → all required stages present and non-empty.
 * Exit 1 → one or more stages missing or empty.
 */
...
if (anyFailed) {
  process.stdout.write(
    'SESSION CHECK FAILED — complete the missing stages before starting Stage 6.\n\n'
  );
  process.exit(1);
} else {
  process.stdout.write(
    'SESSION CHECK PASSED — all required stages are present.\n\n'
  );
  process.exit(0);
}
