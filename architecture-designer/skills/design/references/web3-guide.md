...
- **Principle**: the level of independent scrutiny scales with the value the system can hold or move. This is the "verify first" principle moved into the user's hands, and in this domain it is the primary backstop — it lives in the ecosystem's tools and auditors, not in this plugin.
- **Ask about the target**: what is the maximum value at risk, and therefore what tier of audit is mandatory before any deployment holding real value?
- **Verifies**: a required audit step in the document, sized to value; never implied to be satisfied by generation alone.
