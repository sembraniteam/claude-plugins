...
| .NET                                                         | `dotnet new webapi -n <name> --use-controllers` (or `dotnet new sln` first, for a multi-project solution) — since .NET 8, `dotnet new webapi` without `--use-controllers` scaffolds a controller-less minimal API instead; include the flag when the architecture calls for controller-based routing | `.csproj`, `Program.cs`, a base controller                                    | Models, `DbContext`, additional controllers                                                                                                       |
...
## Prisma worked example

Prisma defines every model in one shared `prisma/schema.prisma` file **by default** — not one file per entity. (Prisma
also supports an opt-in multi-file schema split, `prismaSchemaFolder`, stable since v6.7; if a project has configured
that, treat it like any other multi-file ORM and use one checklist item per entity under the schema folder instead of
everything below.) `implementation-planner`'s "single-schema-file ORM" exception to its one-item-per-entity rule exists
specifically for this default case — see that agent's Step 3 for the rule itself; this is the worked example it points
to.

A project with three ERD entities still gets **one** Data models item, not three, but every entity's model test stays
separate:

```markdown
## Data models

- [ ] `prisma/schema.prisma` — defines `User`, `Order`, and `Product` and their relations per the ERD; run
      `npx prisma generate` after any edit to regenerate the Prisma Client (not itself a checklist item — a generated
      artifact, same `[generated]` convention as a scaffold's own output)

## Configuration

- [ ] `.env.example` — `DATABASE_URL="postgresql://user:password@localhost:5432/dbname"` template

## Setup and run commands

- [ ] `npm run setup` — installs deps, copies `.env.example` → `.env`, runs `npx prisma migrate dev`
- [ ] `npm run dev` — local development server

## Test files

- [ ] `tests/models/user.test.ts` — unit test for the `User` model in `prisma/schema.prisma` (field/relationship
      validation via Prisma Client, not a separate source file)
```

`prisma/migrations/` itself is never a hand-authored checklist item — `npx prisma migrate dev` generates it from
`schema.prisma` the first time the Setup command runs, the same "generator output, not hand-authored" treatment the
Scaffolding section gives a project generator's own files.
