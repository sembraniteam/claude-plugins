...
Within each bounded context, identify aggregates: a cluster of entities and value objects treated as a single unit for
data changes, with one entity as the **aggregate root** (the only member other code is allowed to reference directly)
and a set of **invariants** — business rules that must hold true at the end of every transaction touching the aggregate
(e.g. "an Order's line-item total must equal its stated total," "a StockItem's quantity must never go negative").
...
aggregate and handle cross-aggregate consistency via a domain event or a follow-up command rather than merging them —
the same "smallest change that closes the requirement" bias this plugin applies elsewhere.
