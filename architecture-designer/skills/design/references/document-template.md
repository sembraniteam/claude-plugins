...
11. **Decentralized Architecture Considerations** (only when the Web3 track was active — see `web3-guide.md`) — the confirmed answers to `web3-guide.md`'s eight invariant dimensions (see that file for the canonical list). Every network-specific fact must appear as either a confirmed, sourced value or a `<VERIFY>` placeholder — never asserted from memory. Omit this section entirely when `session.json` has no `web3` key.
