...
| Node.js / NestJS                   | `@nestjs/throttler`                                                                       | `ThrottlerStorageRedisService` from a separate, actively-maintained community package (`@nest-lab/throttler-storage-redis`) — not bundled with `@nestjs/throttler` itself; verify current maintenance status before adopting |
