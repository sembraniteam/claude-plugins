    ...
    **What it does:**
    ...
    ### `/audit-file <path>`
        ...
        **What it does:**
        ...
        (Read + Grep + Glob only)
        │
        │  Returns findings with stable SA-NNN IDs
        ▼
        Audit report in session
        │
        │  User explicitly runs /audit-fix
        ▼
        /audit-fix command
        │
        ├─── Spawns security-fixer agent ──────────────────────────────┐
        │    (Read + Grep + Glob + Write + Edit, no Bash)              │
        │    • Shows fix plan per finding                               │
        │    • Waits for user confirmation                              │
        │    • Applies minimal change per CWE root cause               │
        │    • Outputs fix manifest                                     │
        │                                                               │
        │    [auto-continues, no user action needed]                   │
        │                                                               │
        └─── Spawns fix-reviewer agent ◄────────────────────────────── ┘
        (Read + Grep + Glob only)
        • Reads original report + fix manifest + current files
        • Verdicts: fixed / partially-fixed / not-fixed / introduced-new-issue
        • If not-fixed: sends back to fixer (max 1 retry)
        • If still not-fixed after retry: marks needs-human
        │
        ▼
