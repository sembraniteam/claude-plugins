...
Based on the outcome (single passing hypothesis from Step 4, or the arbitrator's `ONE_WINNER`/`MERGE_FIXES` decision), identify the winning hypothesis id(s) and look up each one's `fix_diff` from the `hN.report.yaml` already read in Step 3 — investigators never commit inside their worktree (see Phase 3 of `../../agents/hypothesis-investigator.md`), so the diff text is the unit of application, applied to the main branch with `git apply`, not a cherry-pick.
...
1. **Verify the artifacts before trusting them** (see `references/apply-verification.md` for why each check below exists). For each winning hypothesis id:
   - Write `fix_diff` to a temp file and dry-run it: `git apply --check <patch_file>`, run from the repo root on the main working tree. If the check fails, do not apply it — escalate to the user with the hypothesis id and the check's error output; a patch that doesn't apply cleanly is not something to hand-resolve automatically.
   - Confirm the patch's contents are consistent with the rest of the report: the `diff --git a/<path> b/<path>` headers inside `fix_diff` must include the report's `test_file`, and must contain none of the paths in `side_effects_flagged` — those were deliberately left out of the captured diff by the investigator (see Phase 3 of `../../agents/hypothesis-investigator.md`). `test_scope_files` is not part of this check. If either condition fails, escalate rather than apply silently — see `references/apply-verification.md` for the full rationale, including why this check and `git apply --check` above are the only integrity signals available now that `fix_diff` has no independent artifact (unlike the old `commit_sha`) behind it.
...
3. Record `pre_apply_sha=$(git rev-parse HEAD)` before applying anything. This is the single rollback point for the rest of this step — use it instead of tracking which patches already applied, regardless of whether the failure happens on the first patch, the second (`MERGE_FIXES`), or the test re-run.
4. Apply the winning patch(es) with `git apply --index <patch_file>` — the `--index` flag stages the change in the same step it lands in the working tree, so `git reset --hard $pre_apply_sha` can cleanly remove it (including any brand-new test file) if a rollback is needed. For `MERGE_FIXES` with two selected hypotheses, apply both patches in sequence, dry-running each with `git apply --check` immediately beforehand — a patch generated against a pristine worktree can still fail to apply once the first hypothesis's patch has already touched the same file.
   - If any `git apply --check` or `git apply --index` in the sequence fails: `git apply` is atomic per invocation (either the whole patch applies or none of it does), but an earlier patch in the same `MERGE_FIXES` sequence that already succeeded is still on the branch. Run `git reset --hard $pre_apply_sha` to return the branch to exactly its pre-apply state regardless of how many patches already landed. Escalate to the user with the conflicting hypothesis ids — do not hand-resolve the conflict yourself.
5. Re-run the tests one more time on the main branch — a fix that passed in an isolated worktree can still fail once applied to the real working tree state. Since the investigator(s) captured the test file(s) inside `fix_diff` alongside the fix(es), applying the patch brings them onto the main branch too, so they are present and runnable.
   ...
   If any test fails, run `git reset --hard $pre_apply_sha` (the same rollback point from step 3, regardless of how many patches were applied) and escalate to the user with the specific failure — do not retry silently.
...
Run this even on the escalation path, once the user's final choice has been applied. This only removes the worktree directory/directories and their branch(es) — nothing was ever committed inside them, so deleting them destroys no fix data. The `.claude/debug-sessions/{session_id}/hN.report.yaml` reports live one level up, outside the worktrees, so they survive cleanup on disk for audit purposes, and each report's `fix_diff` field already holds the full patch text independent of the worktree or branch.
